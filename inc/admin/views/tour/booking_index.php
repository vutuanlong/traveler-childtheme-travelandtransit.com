				...
				foreach ( $posts as $key => $value ) {
					...
					">
						...
						<td class="post-title page-title column-title">
							<strong>
								<a class="row-title" href="<?php echo admin_url( 'edit.php?post_type=st_tours&page=st_tours_booking&section=edit_order_item&order_item_id=' . $post_id ); ?>" title="">
									<?php
									$order = $post_id;
									if ( $order ) {
						...
						<td class="">
							<!--Add by customjob-->
							<?php
							$package_name = get_post_meta( $order, 'package_name', true );
							$item_id      = get_post_meta( $order, 'item_id', true );
							if ( ! empty( $package_name ) ) :
								$people_price_package = STPriceNew::getPeoplePriceByPackage( $item_id, $package_name );
								$price_ori            = $people_price_package['package_price_fixed'];
								echo esc_html( $package_name ) . '( ' . TravelHelper::format_money( $price_ori ) . ' )';
							endif;
							?>
						</td>
						<td class="post-title page-title column-title">
